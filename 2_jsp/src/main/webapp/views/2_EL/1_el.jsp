...
<body>
	...
	<h3>기존 방식 (스크립틀릿, 표현식)</h3>
	 ...
	 <p>
	 	EL을 이용하면 getXXX 메서드를 사용하지 않고 키값만 제시하여 바로 접근 가능<br>
	 	기본적으로 EL은 JSP 내장 객체를 구분하지 않고 모든 내장 객체의 키값을 검색하여 존재하는 경우 그 값을 가져오도록 되어 있다.
	 </p>
	 
	 <p>
	 	* 학원 : ${ academy } <br>
		* 강의장 : ${ classRoom } <br>
		* 내정보 : ${ student.name }, ${ student.age }, ${ student.gender } <br>
		* 내정보 : ${ student2.name }, ${ student2.age }, ${ student2.gender } <br>
	 </p>
	 ...
	 <%-- 
	 	...
	 	공유 범위가 가장 작은 scope 부터 검색을 함
	 	page -> request -> session -> application
	  --%>
	  
	  - 테스트 : ${test } <%-- 찾지 못할 경우 그냥 빈값으로 표시됨 --%>
