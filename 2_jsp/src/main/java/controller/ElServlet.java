...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * => 데이터를 담을 수 있는 공간
		 * 
		 * 1) ServletConext(application scope)
		 * 	- 한 애플리케이션 당 단 1개 존재하는 객체
		 *  - 해당 영역에 데이터를 담을 경우 애플리케이션 전역에서 사용 가능
		 *  
		...
		request.setAttribute("student", new Student("임성준", 26, "남자"));
		
		// session scope에 데이터 담기
		HttpSession session = request.getSession();
		session.setAttribute("academy", "KH");
		session.setAttribute("student2", new Student("김일현", 25, "남자"));
